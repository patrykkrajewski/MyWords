...
#Kolekcja1 ---------------------------------------------------------------------------------------------------
    ...
    def nauka(self, app):
        ...
        self.add_widget(self.kol)
        self.kol = Label(text="Naciśnij przycisk 'Następne pytanie' aby zacząć", pos=(150, 330), color='43CC37',
                         font_size=15)
        self.add_widget(self.kol)
        ...
        self.button = Button(pos=(77, 110), size=(250, 30), text="Następne pytanie!", on_press=self.nxt,
                             on_release=self.sprawdz, bold=True, background_color='#BA0F2F', color='#FFFFFF')
        self.add_widget(self.button)

        self.button_cofnij(self)
    def sprawdz(self, app):
        if len(self.tab_zle) + len(self.tab_dobrze) != len(self.tab):
            self.button_spr = Button(pos=(77, 110), size=(250, 30), text="Sprawdz!", on_press=self.spr, bold=True,
                                     background_color='#BA0F2F', color='#FFFFFF')
    ...
    def nxt(self, app):
        ...
        if self.i < len(self.tab):
            ...
            self.add_widget(self.word)
            self.word_ang = TextInput(pos=(85, 324), size=(236, 42), font_size=18, multiline=False,
                                      background_color='#182A38', foreground_color='#FFFFFF', halign='center',
                                      padding_y=(10, 10))
    ...
    def spr(self, app):
        ...
        if self.i <= len(self.tab) + 1:
            if self.tab[self.i - 2] == self.word_ang.text:
                ...
                self.dob = self.dob + 1
            if self.tab[self.i - 2] != self.word_ang.text:
                self.kol = Label(text="Źle poprawna odpowiedz to: " + self.tab[self.i - 2], pos=(150, 250),
                                 color='#FF0000', font_size=18)
                self.add_widget(self.kol)
    ...
    def slownik(self, app):
        ...
        while i <= len(self.tab):
            if i < 28:
                self.inside_slownik = GridLayout(cols=3, pos=(25, x), size=(350, 20))
                self.ins = Label(text=self.tab[i], color="#00FFCE")
                self.inside_slownik.add_widget(self.ins)
                self.ins_2 = Label(text=self.tab[i + 1], color="#00FFCE")
                self.inside_slownik.add_widget(self.ins_2)
                self.button_usun = Button(text="Usuń", background_color='#FF0000', color='#FFFFFF', size_hint=(0.8, 1),
                                          bold=True, on_press=self.usun_ele)
                self.inside_slownik.add_widget(self.button_usun)
                self.add_widget(self.inside_slownik)
                self.t += 1
            else:
                self.button = Button(text="Następna strona", background_color='#43CC37', color='#FFFFFF', pos=(115, 190),
                             size=(170, 23),
                             on_press=self.nastepna_strona)
                self.add_widget(self.button)
            i += 3
            x -= 20
        if i < 58:
            self.inside = GridLayout(cols=3, pos=(25, 145), size=(350, 40))
            self.textinput_pl = TextInput(text="PL", font_size=16, background_color='#182A38',
                                          foreground_color='#FFFFFF', halign='center', multiline=False,
                                          padding_y=(10, 10), size_hint=(0.5, 1))
            self.inside.add_widget(self.textinput_pl)
            self.textinput_ang = TextInput(text="ANG", font_size=16, background_color='#182A38',
                                           foreground_color='#FFFFFF', halign='center', multiline=False,
                                           padding_y=(10, 10), size_hint=(0.5, 1))
            self.inside.add_widget(self.textinput_ang)
            self.add_widget(self.inside)
            self.button = Button(text="Dodaj słowo:", background_color='#43CC37', color='#FFFFFF', pos=(25, 110),
                                 size=(350, 30),
                                 on_press=self.slownik_dodaj)
            self.add_widget(self.button)

        self.button_cofnij(self)
    def nastepna_strona(self,app):
        self.clear_widgets()
        self.szablon(self)
        ...
        while i <= len(self.tab):
            if i < 58:
                ...
                self.add_widget(self.inside_slownik)
                self.t += 1
                self.button = Button(text="Poprzednia strona", background_color='#43CC37', color='#FFFFFF',
                                     pos=(115, 190),
                                     size=(170, 23),
                                     on_press=self.slownik)
                self.add_widget(self.button)
            i += 3
            x -= 20
        if i < 58:
            ...
            self.button_dodaj_slowo = Button(text="Dodaj słowo:", background_color='#43CC37', color='#FFFFFF', pos=(25, 110),
                                 size=(350, 30),
                                 on_press=self.slownik_dodaj)
            self.add_widget(self.button_dodaj_slowo)


        self.button_cofnij(self)

    def slownik_poza_zakresem(self,app):
        pass
